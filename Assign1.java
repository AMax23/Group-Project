...
//		String [] sources = {"<ENTER JAR FILES DIRECTORY HERE>"};				
		...
		// Loop until user does not want to search for any other types
		while (true){											
			...
			System.out.print("Count number of occurences for a different type (Yes/No): ");
			if (keyboard.nextLine().toLowerCase().equals("no")){
				break;
			}
			targetType = uc.getTargetType();	
		}
	}
}